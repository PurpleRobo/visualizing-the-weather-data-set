...
def line_chart(df,period,col):
    ...
    This function accepts the dataframe df ,period(day/month/year) and col(feature),    which plots the aggregated value of the feature based on the periods. Ensure t      the period labels are properly named.
    ...
    """
    a = df.groupby(pd.Grouper(freq=period[0]))[col].mean()
    plt.plot(a)
...
def plot_categorical_columns(df):
    ...
    for i in range(3):
        for j in range(2):
            sns.distplot(df[df.columns[k]], ax=axs[i][j])
...
def plot_cont(df,plt_typ):
    ...
    if plt_typ == 'distplot':
        ...
        for i in range(3):
            for j in range(2):
                sns.boxplot(df[df.columns[k]], ax=axs[i][j])
                k += 1
...
def group_values(df,col1,agg1,col2):
    ...
    df.groupby(col1)[col2].agg(agg1).plot(kind='bar', figsize=(15, 5))
